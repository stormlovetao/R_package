...
#' @examples
#'
#' output <- gmfp.ac.gpd(known.conf = dat$known.conf, fea.dat = dat$fea.dat, snp.dat = dat$snp.dat,
...
#'   ## When the empirical P-value is less than 0.02, a more accurate empirical P-value is estimated using the GPD fit.
#'   output <- gmfp.ac.gpd(known.conf = dat$known.conf, fea.dat = dat$fea.dat, snp.dat = dat$snp.dat,
#'                  trios.idx = dat$trios.idx[1:10,], cl = cl, cov.pool = dat$cov.pool, nperm = 100, gpd.perm = 0.02)
#'
#'   stopCluster(cl)
#' }
#'
...
#'
gmfp.ac.gpd <- function(snp.dat, fea.dat, known.conf, trios.idx, cl = NULL, cov.pool = NULL,
