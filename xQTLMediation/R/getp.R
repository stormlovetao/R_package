...
#' @title This funciton get permutation statistics
get.perm <- function(treatment, mediator, outcome, confounders, t_stat=NULL, Minperm=100, Maxperm=10000){
	if(is.null(t_stat)){ # direct permutation scheme
	...
	}else{ # adaptive permutation scheme
		t_stat_perm <- c()
		b <- 0
		step <- Minperm
		repeat{
			if(b + step > Maxperm){
				mediator_perm <- matrix(rep(mediator, each = Maxperm-b), byrow = TRUE, ncol = Maxperm-b)
				b <- Maxperm
			}else{
				mediator_perm <- matrix(rep(mediator, each = step), byrow = TRUE, ncol = step)
				b <- b + step
			}
			for (j in 0:2) {
				ind <- which(treatment == j)
			...
			t_stat_perm <- c(t_stat_perm, apply(mediator_perm, 2, get.lm, treatment = treatment,
													outcome = outcome, confounderset = confounders))
			if((b >= Maxperm) || (sum(abs(t_stat) <= abs(t_stat_perm)) >= step))
				break
		}
	}
	return(t_stat_perm)
...
getp.func <- function(i, triomatrix, confounders, Minperm=100, Maxperm=10000, use.gpd = FALSE, gpd.perm = 0.01,
	...
	if(!is.null(pool_cov) && !is.null(est_conf_pool_idx)){
		...
		confounders <- cbind(confounders, pool_cov[, which(est_conf_pool_idx[i, ] == 1)])
	}
	...
	if(Minperm < Maxperm){ # adaptive permutation scheme
		t_stat_perm <- get.perm(treatment=treatment, mediator=mediator, outcome=outcome,
									confounders=confounders, t_stat=t_stat, Minperm=Minperm, Maxperm=Maxperm)
	}else{ # direct permutation scheme
		t_stat_perm <- get.perm(treatment=treatment, mediator=mediator, outcome=outcome,
									confounders=confounders, Maxperm=Maxperm)
	}
